...
class MMad {
public:
    void Compute(LocalTensor<DT>& dstTensor, const LocalTensor<ST>& lhs, LocalTensor<ST>& rhs, uint32_t row, uint32_t col) {
        for (uint32_t r = 0; r < MM_CFG.basicM; r++) {
            for (uint32_t c = 0; c < MM_CFG.basicN; c++) {
                auto dstAddr = dstTensor.GetAddr();
                auto& dstValue = dstAddr[(row * MM_CFG.basicM + r) * MM_CFG.singleCoreN + MM_CFG.basicN * col + c];
